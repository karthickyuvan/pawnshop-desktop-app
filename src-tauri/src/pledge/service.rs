...
//             "INSERT INTO fund_transactions
//             (type,total_amount,module_type,module_id,reference,description,payment_method, transaction_ref,created_at,created_by)
//             VALUES ('ADD',?1,'FEE',?2,?3,?4,?5,?6,?7,?8)",
//             params![
...
//             "INSERT INTO fund_transactions
//             (type,total_amount,module_type,module_id,reference,description,payment_method,transaction_ref,created_at,created_by)
//             VALUES ('ADD',?1,'INTEREST',?2,?3,?4,?5,?6,?7,?8)",
//             params![
//                 req.first_interest_amount,
//                 pledge_id,
//                 pledge_no,
...
//             COALESCE(p.parked_interest, 0.0) as parked_interest,
//             COALESCE(p.last_interest_date, p.created_at) as last_interest_date
//             FROM pledges p
//             JOIN customers c ON p.customer_id = c.id
//             WHERE p.id = ?1
//             ",
...
//                         pledge_date: row.get(5)?,
//                         duration_months: row.get(6)?,
//                         customer_code: row.get(7)?,
//                         customer_name: row.get(8)?,
//                         relation_type: row.get(9)?,
//                         relation_name: row.get(10)?,
//                         phone: row.get(11)?,
//                         address: row.get(12)?,
//                         photo_path: row.get(13)?,
//                         loan_type: row.get(14)?,
//                         scheme_name: row.get(15)?,
//                         interest_rate: row.get(16)?,
//                         price_per_gram: row.get(17)?,
//                         principal_amount: row.get(18)?,
//                         total_gross_weight: row.get(19)?,
...
//     let breakdown = crate::interest::engine::calculate_interest(&pledge, &settings, interest_paid_in_current_cycle);
...
//     let original_parked_interest = pledge.parked_interest;
...
//         interest_pending: breakdown.interest_pending + original_parked_interest, 
//         original_principal,
//         parked_interest: original_parked_interest, 
//     })
// }
...
//             .unwrap_or(0) > 0;
//         if is_bank_mapped {
//             return Err("This pledge is bank-mapped. Please use Bank Unmapping to close this pledge.".to_string());
//         }
//     }
...
//     } else if req.payment_type == "INTEREST" {
...
//             principal_portion = req.amount - total_interest_owed;
//         } else {
//             interest_portion = req.amount;
//             principal_portion = 0.0;
//         }
...
//         return Err(format!("Overpayment not allowed. Total payable: ₹{}", total_payable));
//     }
...
//         tx.execute(
//             "INSERT INTO fund_transactions (type, total_amount, module_type, module_id, reference, description, payment_method, transaction_ref, created_by) VALUES ('ADD', ?1, ?2, ?3, ?4, ?5, 'CASH', ?6, ?7)",
//             params![req.amount, module_type, req.pledge_id, pledge_data.pledge.pledge_no, reference_text, req.reference, req.created_by],
//         ).map_err(|e| e.to_string())?;

//         let fund_tx_id = tx.last_insert_rowid();
//         if let Some(ref denoms) = req.denominations {
//             for (note, qty) in denoms {
//                 tx.execute("INSERT INTO fund_denominations (fund_transaction_id, denomination, quantity, amount) VALUES (?1, ?2, ?3, ?4)", params![fund_tx_id, note, qty, (*note as f64) * (*qty as f64)]).map_err(|e| e.to_string())?;
//             }
//         }
//     } else {
//         let module_type = if req.payment_type == "CLOSURE" { "CLOSURE" } else { "PAYMENT" };
//         tx.execute(
//             "INSERT INTO fund_transactions (type, total_amount, module_type, module_id, reference, description, payment_method, transaction_ref, created_by) VALUES ('ADD', ?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
//             params![req.amount, module_type, req.pledge_id, pledge_data.pledge.pledge_no, format!("Payment for Pledge {}", pledge_data.pledge.pledge_no), req.payment_mode, req.reference, req.created_by],
//         ).map_err(|e| e.to_string())?;
//     }
...
//             tx.execute(
//                 "UPDATE pledges SET parked_interest = ?1 WHERE id = ?2",
//                 params![remaining_parked, req.pledge_id],
//             ).map_err(|e| e.to_string())?;
//         }
//     }
...
pub struct CreatePledgeRequest {
    ...
    pub loan_amount: f64,
    pub created_by: i64,
    pub payments: Vec<PledgePaymentMode>,
    pub processing_fee_amount: f64,
    pub first_interest_amount: f64,
    pub items: Vec<PledgeItemRequest>,
    pub pledge_date: Option<String>,
...
pub fn create_pledge(
    ...
    if user_role != "OWNER" {
        let current_today = Local::now().format("%Y-%m-%d").to_string();
        if pledge_date != current_today {
            return Err("Only OWNER can create pledges with a different date".to_string());
        }
    }
    ...
    let transaction_timestamp = if trimmed_date.contains('T') {
        ...
        format!("{}T{}Z", trimmed_date, chrono::Utc::now().format("%H:%M:%S"))
    };
    ...
    // ── 🟢 INSERT BALANCING WITHDRAWAL RECORDS TO BRING TOTAL LOANS ISSUED TO GROSS PRINCIPAL ──
    if req.processing_fee_amount > 0.0 {
        tx.execute(
            "INSERT INTO fund_transactions
            (type, total_amount, module_type, module_id, reference, description, payment_method, transaction_ref, created_at, created_by)
            VALUES ('WITHDRAW', ?1, 'PLEDGE', ?2, ?3, ?4, 'DEDUCTION', ?5, ?6, ?7)",
            params![
                req.processing_fee_amount,
                pledge_id,
                pledge_no,
                format!("Processing Fee Deduction - {}", receipt_number),
                first_payment_ref,
                transaction_timestamp,
                req.created_by
            ],
    ...
    if req.first_interest_amount > 0.0 {
        tx.execute(
            ...
            params![
                ...
                pledge_id,
                pledge_no,
                format!("First Month Interest Deduction - {}", receipt_number),
                first_payment_ref,
                transaction_timestamp,
                req.created_by
            ],
...
pub fn add_pledge_payment(db: &Db, req: AddPaymentRequest) -> Result<(), String> {
    ...
    if req.payment_mode == "CASH" {
        ...
        tx.execute(
            "INSERT INTO fund_transactions (type, total_amount, module_type, module_id, reference, description, payment_method, transaction_ref, created_at, created_by) VALUES ('ADD', ?1, ?2, ?3, ?4, ?5, 'CASH', ?6, ?7)",
            params![req.amount, module_type, req.pledge_id, pledge_data.pledge.pledge_no, reference_text, req.reference, req.created_by],
        ).map_err(|e| e.to_string())?;
        ...
        let module_type = if req.payment_type == "CLOSURE" { "CLOSURE" } else { "PAYMENT" };
        tx.execute(
            "INSERT INTO fund_transactions (type, total_amount, module_type, module_id, reference, description, payment_method, transaction_ref, created_at, created_by) VALUES ('ADD', ?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8)",
            params![req.amount, module_type, req.pledge_id, pledge_data.pledge.pledge_no, format!("Payment for Pledge {}", pledge_data.pledge.pledge_no), req.payment_mode, req.reference, req.created_by],
        ).map_err(|e| e.to_string())?;
    }
    ...
    let rows = stmt
        .query_map([], |row| {
            ...
            Ok(serde_json::json!({
                "id": row.get::<_, i64>(0)?,
                "pledge_no": row.get::<_, String>(1)?,
                "receipt_number": row.get::<usize, Option<String>>(2)?,
                "customer_name": row.get::<_, String>(3)?,
                "customer_code": row.get::<_, String>(4)?,
                "phone": row.get::<_, String>(5)?,
                "scheme_name": row.get::<_, String>(6)?,
