...
//     // 🟢 1. Opening Balance Calculation (CASH only)
//     let opening_balance: f64 = conn
//     .query_row(
//         "SELECT COALESCE(SUM(
//             CASE WHEN ft.type='ADD' THEN ft.total_amount
...
//         let type_field: String = row.get(2)?;
//         let module_type: String = row.get::<_, Option<String>>(3)?.unwrap_or("OTHER".to_string());
//         let reason: String = row.get::<_, Option<String>>(4)?.unwrap_or("".to_string());
//         let description: String = row.get::<_, Option<String>>(5)?.unwrap_or("".to_string()); 
//         let raw_payment_method: String = row.get(6)?;
//         let amount: f64 = row.get(7)?;
//         let customer_name: Option<String> = row.get(9)?;
//         let transaction_ref: Option<String> = row.get(10)?;

//         // ── 🟢 DYNAMIC PAYMENT METHOD LABELLING FOR DEDUCTION ENTRIES ──
//         let payment_method = if raw_payment_method == "DEDUCTION" {
//             if module_type == "INTEREST" {
//                 "INTEREST".to_string()
//             } else if module_type == "FEE" {
//                 "PROCESSING_FEE".to_string()
//             } else if module_type == "PLEDGE" {
//                 if description.contains("Interest") {
//                     "INTEREST_DEDUCTION".to_string()
//                 } else if description.contains("Fee") {
//                     "FEE_DEDUCTION".to_string()
//                 } else {
//                     "DEDUCTION".to_string()
//                 }
//             } else {
...
pub fn get_daybook(db: &Db, date: String) -> Result<DaybookResponse, String> {
    ...
    let rows = stmt.query_map(params![date], |row| {
        ...
        let created_at: String = row.get(1)?;
        let type_field: String = row.get(2)?;
        let module_type = row.get::<_, Option<String>>(3)?.unwrap_or_else(|| "OTHER".to_string());
        let reason = row.get::<_, Option<String>>(4)?.unwrap_or_else(|| "".to_string());
        let description = row.get::<_, Option<String>>(5)?.unwrap_or_else(|| "".to_string()); 
        let raw_payment_method: String = row.get(6)?;
        let amount: f64 = row.get(7)?;
        let customer_name: Option<String> = row.get(9)?;
        let transaction_ref: Option<String> = row.get(10)?;
