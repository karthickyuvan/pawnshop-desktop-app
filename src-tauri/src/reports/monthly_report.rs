...
pub struct MonthlyReport {
    ...
    // --- 4. OUTFLOWS ---
    pub loans_issued: f64,  // gross loan principal
    pub net_loans_issued:f64,     // net cash disbursed           
    pub expenses: f64,                     
...
pub fn get_monthly_report(
    ...
    let mut metal_stmt = conn.prepare(
        ...
        SELECT
            ...
            COUNT(DISTINCT CASE WHEN strftime('%Y-%m', COALESCE(p.pledge_date, p.created_at)) = ?1 THEN p.id END) as in_count,
            
            -- Outward Pockets Count (Distinct unique p.id instances)
            COUNT(DISTINCT CASE WHEN (p.status = 'CLOSED' AND strftime('%Y-%m', p.closed_at) = ?1) OR (p.status = 'AUCTIONED' AND strftime('%Y-%m', p.auctioned_at) = ?1) THEN p.id END) as out_count
        FROM metal_types mt
