...
pub fn get_bank_mapping_report(
    ...
    let mut stmt = conn.prepare(
        ...
        SELECT
            ...
            b.bank_name,             -- Joined from banks table
            p.loan_amount,
            bm.amount AS mapped_amount, -- Column name is amount in schema
            bm.status
        FROM bank_mappings bm
        JOIN pledges p ON p.id = bm.pledge_id
        JOIN customers c ON c.id = p.customer_id
        JOIN banks b ON b.id = bm.bank_id
        ORDER BY bm.mapped_date DESC -- Column name is mapped_date in schema
        "
    ).map_err(|e| e.to_string())?;

    let rows_iter = stmt.query_map([], |row| {
        let db_status: String = row.get(5)?;
        
        // Map database ACTIVE status code to front-end's MAPPED key for stats calculation
        let status = if db_status == "ACTIVE" {
            "MAPPED".to_string()
        } else {
            db_status
        };
