...
/* ============================================================
   GENERATE EXPENSE CODE BASED ON SELECTED YEAR
============================================================ */

fn generate_expense_code(conn: &rusqlite::Connection, year: &str) -> Result<String> {
    let pattern = format!("EXP-{}-%", year);
    ...
    Ok(format!("EXP-{}-{:04}", year, next_number))
}
...
pub fn create_expense(db: State<Db>, req: CreateExpenseRequest) -> Result<String, String> {
    ...
    // ── 🟢 EXTRACT EXPENSE YEAR DYNAMICALLY FROM SELECTED EXPENSE DATE ──
    let expense_year = if expense_date.len() >= 4 {
        expense_date[..4].to_string()
    } else {
        Local::now().format("%Y").to_string()
    };

    // Generate the incremental sequence code for the target year
    let expense_code =
        generate_expense_code(&conn, &expense_year).map_err(|e| format!("Code generation failed: {}", e))?;
