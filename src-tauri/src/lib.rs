...
use auth::staff::{
    ...
    SalarySettlementSummary,
     get_advance_denominations,
     get_past_salary_payments,
    PastSalaryPaymentRow,
};
// Audit
...
use crate::jewellery_types::jewellery_type::{
...
use crate::metal_types::metal_type::{
    create_metal_type,
    get_metal_types,
    toggle_metal_type,
    get_active_metal_types,
    MetalType,
};
...
use reports::metal_movement_report::get_metal_movement_report_cmd;
...
use reports::auction_report::get_auction_report_cmd;
use reports::investor_report::get_global_investor_transactions_cmd;
...
#[tauri::command]
fn get_salary_advances_cmd(
    db: State<Db>,
    user_id: i64,
) -> Result<Vec<StaffSalaryAdvance>, String> {

    get_salary_advances(&db, user_id)
}

#[tauri::command]
fn get_salary_summary_cmd(
    db: State<Db>,
    user_id: i64,
) -> Result<SalarySettlementSummary, String> {

    get_salary_summary(&db, user_id)
}

#[tauri::command]
fn create_salary_payment_cmd(
    db: State<Db>,
    user_id: i64,
    salary_month: String,
    payment_mode: String,
...
#[tauri::command]
fn get_advance_denominations_cmd(
    db: State<Db>,
    advance_id: i64,
) -> Result<Vec<(i32, i32)>, String> {
    get_advance_denominations(&db, advance_id)
}

#[tauri::command]
fn get_past_salary_payments_cmd(
    db: State<Db>,
    user_id: i64,
) -> Result<Vec<PastSalaryPaymentRow>, String> {
    get_past_salary_payments(&db, user_id)
}
...
#[tauri::command]
fn create_investor_cmd(
    db: State<Db>,
    request: investors::service::CreateInvestorRequest,
) -> Result<(), String> {
    investors::service::create_investor(&db, request)
        .map_err(|e| e.to_string())
}

#[tauri::command]
fn get_investors_cmd(
    db: State<Db>,
) -> Result<Vec<investors::service::Investor>, String> {
    investors::service::get_investors(&db)
        .map_err(|e| e.to_string())
}


#[tauri::command]
 fn update_investor_cmd(
    db: tauri::State<Db>,
    request: UpdateInvestorRequest,
) -> Result<(), String> {

    investors::service::update_investor(
        &db,
        request,
    )
    .map_err(|e| e.to_string())
...
#[tauri::command]
fn get_investor_by_id_cmd(
    db: State<Db>,
    investor_id: i64,
) -> Result<investors::service::Investor, String> {
    investors::service::get_investor_by_id(
        &db,
        investor_id,
    )
    .map_err(|e| e.to_string())
...
#[tauri::command]
fn create_investment_cmd(
    db: State<Db>,
    request: investors::service::CreateInvestmentRequest,
) -> Result<(), String> {
    investors::service::create_investment(
        &db,
        request,
    )
    .map_err(|e| e.to_string())
...
#[tauri::command]
fn get_investor_ledger_cmd(
    db: tauri::State<Db>,
    investor_id: i64,
) -> Result<InvestorLedgerResponse, String> {

    investors::service::get_investor_ledger(
        &db,
        investor_id,
    )
    .map_err(|e| e.to_string())
...
#[tauri::command]
fn withdraw_investment_cmd(
    db: State<Db>,
    request: WithdrawInvestmentRequest,
) -> Result<(), String> {

    investors::service::withdraw_investment(
        &db,
        request,
    )
    .map_err(|e| e.to_string())
...
#[tauri::command]
 fn toggle_investor_status_cmd(
    db: State<Db>,
    request: ToggleInvestorStatusRequest,
) -> Result<(), String> {
    investors::service::toggle_investor_status(
        &db,
        request,
    )
    .map_err(|e| e.to_string())
...
fn get_investor_interest_preview_cmd(
    db: State<Db>,
    investor_id: i64,
) -> Result<investors::service::InvestorInterestPreview, String> {
    investors::service::get_investor_interest_preview(&db, investor_id)
        .map_err(|e| e.to_string())
}
...
fn get_investors_interest_due_report_cmd(
    db: State<Db>,
) -> Result<Vec<investors::service::InvestorInterestPreview>, String> {
    let investors = investors::service::get_investors(&db).map_err(|e| e.to_string())?;
    let mut report = Vec::new();
    for inv in investors {
        if inv.is_active {
            if let Ok(preview) = investors::service::get_investor_interest_preview(&db, inv.id) {
                if preview.total_months > 0 {
                    report.push(preview);
                }
            }
        }
...
// ── DRAWER DENOMINATION EXCHANGE (NEW) ───────────────────────────────────────
#[tauri::command]
fn process_drawer_exchange_cmd(
    db: State<Db>,
    created_by: i64,
    amount: f64,
    ...
    fund_management::service::process_denomination_exchange(&db, created_by, amount, received_denominations, given_denominations)
}
...
#[tauri::command]
fn get_auctioned_list_cmd(
    db: State<Db>,
    search: Option<String>,
) -> Result<auction::service::AuctionedListResponse, String> {
    auction::service::get_auctioned_pledges(&db, search)
}
...
fn mark_pledge_auctioned_cmd(
    ...
    auction::service::mark_pledge_auctioned(
        &db,
        pledge_id,
        auction_amount,
        auction_notes,
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_staff_cmd,
            toggle_staff_cmd,
            create_salary_advance_cmd,
            get_salary_advances_cmd,
            get_salary_summary_cmd,
            create_salary_payment_cmd,
            get_advance_denominations_cmd,
            get_past_salary_payments_cmd,
            // Audit
            get_audit_logs_cmd,
            // Metal Types
