    ...
    let mut stmt = conn
        .prepare(
            ...
            SELECT
                ...
                s.interest_rate,
                s.interest_type,
                s.processing_fee_type,
                s.processing_fee_value,
                s.is_active
            FROM schemes s
            JOIN metal_types m
            ON m.id = s.metal_type_id
            ORDER BY s.id DESC
            ",
        )
        .map_err(|e| e.to_string())?;
...
pub fn get_schemes(
    ...
    get_schemes_internal(&db)
}

/* ================= TOGGLE ================= */

fn toggle_scheme_internal(
    db: &Db,
    scheme_id: i64,
    is_active: bool,
) -> Result<(), String> {
