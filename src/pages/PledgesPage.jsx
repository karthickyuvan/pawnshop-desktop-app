...
//          pledge_date:
//     response.fullDetails?.pledge?.pledge_date,

//   created_at:
//     response.fullDetails?.pledge?.created_at,
//         payload,
//         customer,
//         selectedScheme,
//         finalLoan,
...
//                         onChange={(e) =>
//                           updateDenomination(note, e.target.value)
//                         }
//                       />
//                     </div>
//                   </div>
...
//                     textAlign: "center",
//                     background:
//                       totalCash === netDisbursed ? "#ecfdf5" : "#fff1f2",
...
export default function PledgesPage({ user, setActiveMenu }) {
  ...
  const handleAutoFill = async () => {
    const response = await autoFillDenominations(netDisbursed);
    if (!response.success) {
      toast.error(t("drawer_insufficient_cash", "Not enough physical cash available in the vault drawer stock.")); // 🚀 Upgraded alert
      return;
    }
    setDenominations(response.denominations);
  ...
  const handleSave = async () => {
    if (!customer) {
      toast.error(t("select_customer_error", "Please select a valid customer profile reference.")); // 🚀 Upgraded alert
      return;
    }
    if (!loanInfo.schemeId) {
      toast.error(t("select_scheme_error", "Please explicitly select a calculation valuation scheme.")); // 🚀 Upgraded alert
      return;
    }
    if (items.length === 0) {
      toast.error(t("add_items_error", "Pledge cannot be created with zero item assets recorded.")); // 🚀 Upgraded alert
      return;
    }
    ...
    try {
      ...
      console.error("SAVE ERROR:", e);
      toast.error(t("save_failed", "Failed to book pledge operation: ") + (e?.message || e)); // 🚀 Upgraded alert
    }
  };
