...
export default function SettingsPage() {
  ...
  useEffect(() => {
    invoke("get_shop_settings")
      ...
      .catch(err => {
        ...
        toast.error(t("failed_to_load_settings", "Failed to retrieve baseline configuration logs."));
      });
  }, [t]);
  ...
  const handleLogoChange = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      setLogoBase64(b64);                 // raw base64 for backend
      setLogoExt(ext);
      toast.success(t("logo_attached", "New logo attached to staging buffer."));
    };
    reader.readAsDataURL(file);
  };
  ...
  // ── Save shop profile ──────────────────────────────────────────────────
  const handleSaveShop = async () => {
    if (!shop.shop_name?.trim() || !shop.address?.trim() || !shop.phone?.trim()) {
      const fieldErr = t("shop_required_fields", "Shop name, full address, and phone parameters are mandatory.");
      setShopMsg({ text: fieldErr, ok: false });
    ...
    try {
      const updated = await invoke("save_shop_settings", {
        payload: {
          ...
          license_number: shop.license_number?.trim() || null,
          logo_base64:    logoBase64 || null,
          logo_extension: logoExt    || null,
        },
      });
      ...
      const successText = t("shop_save_success", "Shop profile saved successfully.");
      setShopMsg({ text: successText, ok: true });
      toast.success(successText); // 🚀 Fire layout toast confirmation
      setTimeout(() => setShopMsg({ text: "", ok: true }), 3500);
    } catch (err) {
      const errMsg = String(err);
      setShopMsg({ text: errMsg, ok: false });
      toast.error(errMsg);
    } finally {
      setShopSaving(false);
    }
  ...
  const handleChangePassword = async () => {
    setPwMsg({ text: "", ok: true });
    if (!passwords.current) {
      const errTxt = t("enter_current_password", "Enter your current password.");
      setPwMsg({ text: errTxt, ok: false });
      toast.error(errTxt);
      return;
    }
    if (passwords.newPass.length < 8) {
      const errTxt = t("password_min_length", "New password must be at least 8 characters.");
      setPwMsg({ text: errTxt, ok: false });
      toast.error(errTxt);
      return;
    }
    if (passwords.newPass !== passwords.confirm) {
      const errTxt = t("passwords_dont_match", "Passwords do not match.");
      setPwMsg({ text: errTxt, ok: false });
      toast.error(errTxt);
      return;
    }
    ...
    try {
      ...
      setPwMsg({ text: msg, ok: true });
      toast.success(msg || t("password_changed_success", "Credentials updated securely.")); // 🚀 Success target feedback
      setPasswords({ current: "", newPass: "", confirm: "" });
      setTimeout(() => setPwMsg({ text: "", ok: true }), 4000);
    } catch (err) {
      const errMsg = String(err);
      setPwMsg({ text: errMsg, ok: false });
      toast.error(errMsg);
    } finally {
      setPwSaving(false);
    }
