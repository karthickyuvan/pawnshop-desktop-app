...
//               <option value="AUCTIONED">Auctioned</option> {/* Added here */}
...
export default function ViewPledgesPage({ user, setActiveMenu }) {
  ...
  const fetchPledges = async () => {
    try {
      ...
      console.error("Error fetching pledges:", error);
      toast.error(t("failed_load_pledges", "Failed to retrieve pledge logs."));
    }
  };
  ...
  const handlePrintPledge = async (pledgeId) => {
    try {
      ...
      setPrintData(formatted);
      setShowPrintModal(true);
      toast.success(t("pledge_print_staged", "Pledge document sent to print layout."));
  
    } catch (error) {
      console.error("❌ Print error:", error);
      toast.error(t("print_load_failed", "Failed to load pledge properties for printing.")); // 🚀 Upgraded alert
    }
  };
