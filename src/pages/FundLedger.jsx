...
export default function FundLedger({ refreshTrigger, user }) {
  ...
  const translateDescription = (desc) => {
    ...
    // 5. Check for "Denomination Exchange (Outward)"
  ...
  // ✅ Dynamic range clamping to prevent staff from selecting > 7 days
  ...
  const filteredRows = useMemo(() => {
    return rows.filter((r) => {
      ...
      // SECURITY FILTER: Hide investor and payroll-sensitive transactions from staff
      if (isStaff) {
        const ref = (r[3] || "").toUpperCase();    
        const desc = (r[4] || "").toUpperCase();   
        const method = (r[6] || "").toUpperCase(); 
  ...
  const resetFilters = () => {
    ...
    setTypeFilter("ALL");
    setCurrentPage(1);
    toast.success(t("filters_reset", "Ledger query limits dropped back to defaults.")); 
  };
              ...
              paginatedRows.map((r) => {
                ...
                return (
                  <tr key={r[0]} className={isAuctionItem ? "auction-ledger-row" : ""}>
                    ...
                    {/* Translates English strings dynamic reference contents nicely */}
                    <td style={{ color: isAuctionItem ? "#b45309" : "#475569", fontWeight: isAuctionItem ? 600 : 400 }}>
                      {translateDescription(r[4])}
                    </td>
                  </tr>
                );
              })
