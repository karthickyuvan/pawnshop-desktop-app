...
//     if (!targetPledge) {
//       setConfirmModal({
//         message: "Please enter a pledge number",
//         onConfirm: null
//       });
//       return;
//     }
...
//     } catch {
//       setConfirmModal({
//         message: `Pledge "${targetPledge}" not found. Please check the pledge number alignment.`,
//         onConfirm: null
//       });
...
//           console.log("🏦 BANK MAPPING PAYLOAD (from BankMappingPage.jsx):");
//           console.log(JSON.stringify(payload, null, 2));
//           console.log("💰 Denominations:", payload.denominations);
//           console.log("📊 mapDenominations state:", mapDenominations);
...
//         {pledgeDetails && (
//           <div
//             className={`pledge-details-card ${pledgeConfirmed ? "confirmed" : ""}`}
//           >
...
//                 <div
//                   className={`calc-row ${isSurplus() ? "surplus" : "deficit"}`}
//                 >
//                   <span>
//                     <strong>
//                       {isSurplus() ? (
...
export default function BankMappingPage() {
  ...
  useEffect(() => {
    getBanks().then(setBanks).catch((err) => {
      console.error(err);
      toast.error("Failed to load bank accounts registries.");
    });
  }, []);
  ...
  const searchPledge = async () => {
    ...
    if (!targetPledge) {
      toast.error(t("enter_pledge_number_error", "Please enter a pledge number to query.")); // 🚀 Replaced confirmation window error fallback
      return;
    }
    try {
      setSearching(true);
      ...
      toast.error(`Pledge "${targetPledge}" not found. Please verify query input strings.`); // 🚀 Upgraded missing index logs to clean error toasts
      setPledgeDetails(null);
      setPledgeConfirmed(false);
      setMode(null);
    } finally {
  ...
  const submitMapping = async () => {
    if (!pledgeConfirmed || !bankId || !bankLoanAmount || !actualReceived) {
      toast.error(t("fill_required_fields", "Please complete all mandatory parameters and lock down pledge details first.")); // 🚀 Upgraded alert
      return;
    }
    if (paymentMethod !== "CASH" && !mapReference.trim()) {
      toast.error(`Please provide the valid reference hash context for ${paymentMethod} transfers.`); // 🚀 Upgraded alert
      return;
    }
    if (paymentMethod === "CASH") {
      const denomTotal = calcDenomTotal(mapDenominations);
      if (denomTotal === 0) {
        toast.error(t("enter_denominations_error", "Please record currency notes values before confirming settlement submission.")); // 🚀 Upgraded alert
        return;
      }
      if (Math.abs(denomTotal - parseFloat(actualReceived)) > 0.01) {
        toast.error(`Currency values (₹${denomTotal.toFixed(2)}) must exactly correspond to Actual Received metrics (₹${parseFloat(actualReceived).toFixed(2)}).`); // 🚀 Upgraded alert
        return;
      }
    }
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          // 🚀 Converted successful mapping message block from native dialog to dynamic success toast feedback
          toast.success(
            `Bank Mapping Successful!\nNet Received: ₹${netReceived.toFixed(2)} (${
              isSurplus()
                ? `Surplus: +₹${difference.toFixed(2)}`
          ...
          toast.error(t("mapping_failed_alert", "Mapping failed. Please recheck balance properties and parameters.")); // 🚀 Upgraded alert
          console.error("❌ Mapping error:", err);
        } finally {
          setLoading(false);
        }
  ...
  const submitUnmapping = async () => {
    if (!pledgeConfirmed) {
      toast.error(t("confirm_pledge_first", "Please lock down system pledge authentication details first.")); // 🚀 Upgraded alert
      return;
    }
    ...
    if (bankSettleMethod !== "CASH" && !bankSettleReference.trim()) {
      toast.error(`Please supply clear trace indices parameters for ${bankSettleMethod} channels.`); // 🚀 Upgraded alert
      return;
    }
    if (bankSettleMethod === "CASH") {
      const denomTotal = calcDenomTotal(bankSettleDenominations);
      if (denomTotal === 0) {
        toast.error(t("enter_denominations_error", "Please provide a denomination breakdown.")); // 🚀 Upgraded alert
        return;
      }
      if (Math.abs(denomTotal - totalToPay) > 0.01) {
        toast.error(`Denomination totals (₹${denomTotal.toFixed(2)}) must exactly align with Payout constraints (₹${totalToPay.toFixed(2)}).`); // 🚀 Upgraded alert
        return;
      }
    }
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          // 🚀 Replaced unmapping alert prompt with clean success toast feedback
          toast.success(`Bank Unmapping & Closure Successful!\nTotal Paid: ₹${totalToPay.toLocaleString()}`, { duration: 5000 });
          cancelPledge();
        } catch (err) {
          toast.error("Unmapping operation failed: " + (err?.message || err)); // 🚀 Upgraded alert
          console.error(err);
        } finally {
          setLoading(false);
        }
