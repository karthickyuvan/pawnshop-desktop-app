...
export default function DaybookPage({ user }) {
  ...
  // State to hold physical counts entered live by staff
  const [physicalDenoms, setPhysicalDenoms] = useState({});
  ...
  // Sync local input state if the global date gets modified programmatically
  useEffect(() => {
    setLocalDate(date);
  }, [date]);

  // 🕵️‍♂️ Safe Validation Handler
  const handleDateChange = (e) => {
    const value = e.target.value;
    setLocalDate(value); // Keep input perfectly fluid for typing and dropdowns
    ...
    // Only update backend state when a fully formed 4-digit year exists
    // and does not contain temporary padding characters (like "00")
    if (year && year.length === 4 && !year.startsWith("00")) {
      setDate(value);
    }
  };

  // 🔔 IMPORTANT: Instead of crashing the DOM layout with an early return loading screen,
  // we check if we have data to display. This keeps the input field mounted and in focus!
  if (!data && loading)
  ...
  const grouped = data.entries.reduce((acc, entry) => {
    ...
    const formattedEntry = {
      ...entry,
      time: formatTimeIST(entry.time), // 🟢 Direct clean reuse!
    };
  ...
  // ── expected denominations map (from backend data) ──
  const expectedDenoms = {};
  data.denominations?.forEach((item) => {
    expectedDenoms[item.denomination] = item.quantity;
  });

  // ── Calculate physical cash live as user inputs quantities ──
  const actualCash = [500, 200, 100, 50, 20, 10, 5, 2, 1].reduce(
    (sum, d) => sum + (physicalDenoms[d] || 0) * d,
    0
  );
  ...
  // ── Calculate total digital balance ──
  const totalDigital = 
  ...
  return (
    <div className="audit-container">
      {/* 🌟 Visual Indicator: Shows user when background validation is fetching */}
      {loading && (
      ...
      <header className="audit-header">
        ...
        <div className="header-actions">
          {/* Changed value to localDate and assigned our hybrid handler */}
      ...
      <div className={`audit-layout ${loading ? "layout-loading" : ""}`}>
        ...
        <aside className="audit-sidebar">
          ...
          {/* Digital Balances with Cumulative Total */}
          ...
          {/* Interactive Denomination Table for Auditing */}
          {!isStaff && (
            <section className="card denom-card" style={{ padding: "16px" }}>
              <DenominationTable 
                data={physicalDenoms} 
                setData={setPhysicalDenoms} 
                expectedData={expectedDenoms} 
              />
            </section>
          )}
        </aside>
