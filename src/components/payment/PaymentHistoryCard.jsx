...
//     const modeMatch = selectedMode === "ALL" || p.payment_mode === selectedMode;

//     const typeMatch = selectedType === "ALL" || p.payment_type === selectedType;
...
//   const paginatedPayments = filteredPayments.slice(indexOfFirst, indexOfLast);
...
//                   <option value="AUCTIONED">Auction Liquidation</option>
//                 </select>
//               </div>
//             </div>
//           </div>
...
export default function PaymentHistoryCard() {
  ...
  // ✅ Fetch data safely with error toasts
  const fetchPayments = useCallback(async (mode, start, end) => {
    setLoading(true);
    try {
      let data;
      ...
      setPayments(data.payments || []);
      setTotal(data.total_collected || 0);
    } catch (err) {
      console.error("Error fetching payments:", err);
      toast.error("Failed to load payment history logs."); // 🚀 Async Failure Toast
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    fetchPayments(filterMode, startDate, endDate);
  }, [fetchPayments]); // safely added dependency
  ...
  function handleApplyCustomFilter() {
    if (!startDate || !endDate) {
      toast.error("Please select both start and end dates."); // 🚀 Validation Guard
      return;
    }
    if (new Date(startDate) > new Date(endDate)) {
      toast.error("Start date cannot be after the end date."); // 🚀 Validation Guard
      return;
    }
    fetchPayments("custom", startDate, endDate);
  }
  ...
  function handleRefresh() {
    fetchPayments(filterMode, startDate, endDate);
    toast.success("Payment logs updated!"); // 🚀 Refresh acknowledgement
  }

  function handleExport() {
    if (payments.length === 0) {
      toast.error("No records available to export."); // 🚀 Guard Toast
      return;
    }
    try {
      exportPaymentsToCSV(payments);
      toast.success("CSV file exported successfully!"); // 🚀 Success Acknowledgement
    } catch (err) {
  ...
  return (
    <div className="history-card">
      ...
      <div className="history-top">
        ...
        <div className="history-actions">
          <button
            className={`filter-btn ${showFilters ? "active-filter" : ""}`}
            onClick={() => setShowFilters(!showFilters)}
          >
            <Filter size={16} />
            Filters
            ...
            {loading ? (
              ...
              paginatedPayments.map((p, index) => (
                <tr key={index}>
                  ...
                  <td className="customer-cell">{p.customer_name}</td>
                  <td>
                    <span className={`badge badge-type badge-${p.payment_type.toLowerCase()}`}>
                      {p.payment_type}
                    </span>
                  </td>
                  <td>
                    <span className={`badge badge-mode badge-${p.payment_mode.toLowerCase()}`}>
                      {p.payment_mode}
                    </span>
                  </td>
                  <td className="amount-cell text-right">
