...
// ── Auctioned Pledge View (Safety Lockout View) ───────────────────────────
function AuctionedPledgeView({ pledge, onChangePledge }) {
  const { t } = useLanguage();
  ...
  return (
    <div className="pledge-panel">
      ...
      {/* 🛑 CRITICAL SAFETY LOCKOUT BANNER */}
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: "1.15rem", fontWeight: "800", color: "#991b1b" }}>
            Pledge Already Auctioned
          </div>
          <div style={{ color: "#b91c1c", fontSize: "0.85rem", marginTop: "2px" }}>
            This item has been liquidated. No calculations, interest additions, or cash collection entries are permitted.
          </div>
        </div>
      </div>

      {/* Customer Info Card for verification */}
      <div className="customer-card">
          ...
          <div className="customer-info-area">
            <div className="info-row">
              <div>
                ...
                <h3 className="customer-id">{pledge.customer_code || pledge.customer_id}</h3>
              </div>
              <div>
        ...
        <div className="loan-section">
          <div className="loan-grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
            <div>
              <span className="label">Auction Loan Principal</span>
              <h2 style={{ color: "#475569" }}>₹{pledge.principal_amount?.toLocaleString() || pledge.loan_amount?.toLocaleString()}</h2>
            </div>
            <div>
              <span className="label">Original Pledge Scheme</span>
              <h3 style={{ margin: "4px 0 0 0" }}>{pledge.scheme_name} ({pledge.interest_rate}%)</h3>
            </div>
          </div>
        </div>
      </div>
...
export default function PledgePaymentPanel({ pledgeId, onChangePledge }) {
  ...
  useEffect(() => {
    ...
    invoke("get_single_pledge_cmd", { pledgeId: Number(pledgeId) })
      ...
      .catch((err) => {
        ...
        toast.error("Failed to load pledge detail structures.");
      });
  }, [pledgeId]);
  ...
  const isAuctioned  = pledge.status === "AUCTIONED" || (pledge.years_elapsed >= 3 && pledge.principal_amount === 0);
  const isBankMapped = pledge.is_bank_mapped === true;

  // Intercept Render Paths First
  if (isClosed) {
    return <ClosedPledgeView pledge={pledge} payments={payments} onChangePledge={onChangePledge} />;
  }

  if (isAuctioned) {
    return <AuctionedPledgeView pledge={pledge} onChangePledge={onChangePledge} />;
  }
  ...
  const getDaysSince = (dateString) => {
    ...
    const pledgeDate = new Date(dateString);
    const today = new Date();
    const diffTime = today - pledgeDate;
    return Math.floor(diffTime / (1000 * 60 * 60 * 24));
  };
  
  const daysSincePledge = getDaysSince(pledge?.pledge_date || pledge?.created_at);
  ...
  async function handleProcessPayment() {
    // Basic verification guards prior to submission invoking
    if (paymentModes.some(mode => mode.method !== "CASH" && !mode.reference.trim())) {
      toast.error("Please fill in transaction reference ID tags for non-cash modes.");
      return;
    }
    ...
    try {
      await invoke("add_pledge_payment_cmd", {
        req: {
          ...
          denominations: paymentModes[0].method === "CASH"
            ...
            : null,
          is_principal_only: paymentType === "PRINCIPAL_ONLY",
        },
      });

      toast.success("Payment registered successfully!"); // 🚀 Success Feedback Toast
      ...
      toast.error("Payment Processing Error: " + String(error)); // 🚀 Replaced dialog plugin
    }
  }
    ...
    <div className="pledge-panel">
        ...
        <div className="loan-section">
          ...
          <div className="loan-grid">
            ...
            <div>
              <Calendar size={18} />
              <span className="label">{t("pledge_date")}</span>
              <h3>{formatDateIST(pledge.pledge_date || pledge.created_at)}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="pending-box">
        <div>
          ...
          <small style={{ color: "#9c194d", display: "block", marginTop: "4px" }}>
            {daysSincePledge} {t("days_since_pledge")}
          </small>
        </div>
      ...
      <div className="payment-details-wrapper">
        ...
        <div className="payment-body">
          ...
          <div className="payment-type-grid">
            ...
            <div
              className={`type-card ${paymentType === "PRINCIPAL_ONLY" ? "active" : ""}`}
              onClick={() => {
            ...
            >
              <div className="type-icon" style={{ background: "#ffedd5", color: "#ea580c", border: "1px solid #fdba74", display: "flex", alignItems: "center", justifycontent: "center", width: "40px", height: "40px", borderRadius: "50%", margin: "0 auto 10px", fontSize: "1.2rem" }}>
                📉
              </div>
              <h4>Principal Only</h4>
              <span style={{ fontSize: "0.75rem", color: "#ea580c", fontWeight: "600" }}>Park Pending Interest</span>
            </div>
        ...
        <div className="collection-body">
          ...
          <div className="collection-summary">
            ...
            {paymentType === "PRINCIPAL_ONLY" && (
              <div className="partial-note" style={{ color: "#ea580c", background: "#fff7ed", border: "1px solid #fed7aa", padding: "10px", borderRadius: "6px", marginTop: "10px", lineHeight: "1.4" }}>
                <strong>Note:</strong> This amount will be deducted entirely from the Principal. The pending interest (₹{pendingInterest.toLocaleString()}) will be parked and added to the final closure amount. The interest calculation date will reset to today.
              </div>
            )}
          </div>
        </div>
      ...
      <div className="action-buttons">
        ...
        <button
          ...
          disabled={
            ...
            (!["PARTIAL", "PRINCIPAL_ONLY"].includes(paymentType) && balance !== 0) ||
            hasCashMismatch
          }
