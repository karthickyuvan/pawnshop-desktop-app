...
import { autoFillDenominations } from "../../utils/cashDenominationManager"; // ── 🟢 Imported Auto Fill Helper
...
export default function WithdrawFundModal({
  ...
  const handleAutoFill = async () => {
    ...
    if (targetAmount <= 0) {
      toast.error("Please enter a valid Amount first to auto-fill.");
      return;
    }
    ...
    setDenoms(response.denominations);
  ...
  const hasRequiredFields = amount && Number(amount) > 0;
  const isBalanceSufficient = Number(amount) <= balance;
  const hasValidRef = !needsRef || (transactionRef && transactionRef.trim() !== "");
  const isCashMatched = !isCash || (finalAmount > 0 && finalAmount === totalCash);

  // ── 🟢 DYNAMIC VALIDATION GUARD EVALUATION FOR SUBMIT BUTTON ──
  const canWithdraw = hasRequiredFields && isBalanceSufficient && hasValidRef && isCashMatched;
      ...
      <div className="modal">
        ...
        {isCash && (
          <div className="modal-section" style={{ marginTop: "20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
              <label className="fund-label" style={{ margin: 0 }}>Cash Denominations</label>
              
              {/* ── 🟢 Auto Fill Button triggers optimal vault split distribution ── */}
              <button
                type="button"
              ...
              >
                ✨ Auto Fill (₹{Number(amount || 0).toLocaleString()})
              </button>
            </div>
        ...
        <div className="modal-actions">
          ...
          <button
            ...
            disabled={!canWithdraw} // ── 🟢 Disable if canWithdraw is false
            style={{
              opacity: canWithdraw ? 1 : 0.6,
              cursor: canWithdraw ? "pointer" : "not-allowed",
              backgroundColor: canWithdraw ? "#dc2626" : "#9ca3af" // Gray out when disabled
            }}
          >
            Withdraw
          </button>
