...
// console.log("pledge_date =", data?.pledge_date);
...
//      ════════════════════════════════════════════════════════ */
//   const renderMainReceipt = (copyLabel) => (
...
//         <div className="right-date">Pledged Date: {getPledgeDate()}</div>
//       </div>
...
//             <div className="pocket-detail-row"><span>Date:</span><span>{getPledgeDate()}</span></div>
...
//           <div className="vertical-item">Date: {getPledgeDate()}</div>
...
//           {renderMainReceipt("CUSTOMER COPY")}
//           {renderMainReceipt("STORE COPY")}
//           {renderPocketCopy()}
//         </div>
//       </div>
//     </div>
...
// import { useLanguage } from "../context/LanguageContext"; // Adjust this relative path as needed for your project structure
import "./PledgePrintModal.css";
...
export default function PledgePrintModal({ data, shopSettings, onClose, isReprint = false }) {
  ...
  /* ════════════════════════════════════════════════════════
     MAIN RECEIPT — UPDATED LAYOUT WITH TRANSLATIONS & RECEIPT NO
     ════════════════════════════════════════════════════════ */
  // const renderMainReceipt = (copyLabelKey, defaultCopyLabel) => (
  //   <div className="sketch-receipt">
  ...
  //         <h2>{shopSettings?.shop_name || "COMPANY NAME"}</h2>
  //         <p>{shopSettings?.address || "Address"}</p>
  //         <p>{t("Ph No")}: {shopSettings?.phone || "—"} | {shopSettings?.email || ""}</p>
  //       </div>
  //       <div className="copy-badge-area">
  //         <span className="badge">{t(copyLabelKey)}</span>
  //       </div>
  //     </div>

  //     {/* 2. Title Bar: Displays "Pledge Receipt" and "Receipt No" next to each other */}
  //     <div className="sketch-title-bar">
  //       <div className="left-title">
  //         {t("Pledge Receipt")} / {t("Receipt No")}: {receiptNumber}
  //       </div>
  //       <div className="right-date">{t("Pledged Date")}: {getPledgeDate()}</div>
  //     </div>
  ...
  //       {/* Left Side: Customer Text + Highlight Boxes */}
  //       <div className="customer-info-box">
  //         <div className="box-title">{t("Customer Informations")}</div>
  //         <div className="info-content-flex">
            
  //           {/* BIG Customer Text */}
  //           <div className="info-text">
  //             <p><b>{t("Name")}:</b> {data.customer?.name || "—"}</p>
  //             <p>
  //               <b>{t("Relation Name")}:</b>{" "}
  //               {data.customer?.relation_type && data.customer?.relation_name
  //                 ? `${data.customer.relation_type} ${data.customer.relation_name}`
  //                 : t("Self")}
  //             </p>
  //             <p className="address-text"><b>{t("Address")}:</b> {data.customer?.address || "—"}</p>
  //             <p><b>{t("Ph No")}:</b> {data.customer?.phone || "—"}</p>
  //           </div>
            
  //           {/* Single Line Highlights (Loan & Pledge No) */}
  //           <div className="info-highlights-vertical">
  //             <div className="hl-box-vert"><b>{t("Pledge No")}:</b> {data.pledgeNo}</div>
  //             <div className="hl-box-vert"><b>{t("Loan Amount")}:</b> ₹{Number(data.finalLoan || 0).toLocaleString()}</div>
  //           </div>
  ...
  //       {/* Right Side: Both Photos side-by-side */}
  //       <div className="photos-box">
  //         <div className="box-title">{t("Photos")}</div>
  //         <div className="photos-wrapper">
  ...
  //               <img src={customerPhoto} alt="Customer" className="stamp-photo" />
  //             ) : (
  //               <div className="stamp-photo no-photo-placeholder">{t("No Photo")}</div>
  //             )}
  //             <span className="photo-label">{t("Customer")}</span>
  //           </div>
  ...
  //               <div className="stamp-photo no-photo-placeholder">{t("No Photo")}</div>
  //             )}
  //             <span className="photo-label">{t("Jewel")}</span>
  //           </div>
  ...
  //     {/* 4. Item Details (Single Inline Row) */}
  //     <div className="item-details-box">
  //       <div className="box-title">{t("Jewel Details")}</div>
  //       <div className="item-details-single-line">
  //         <div className="detail-desc">
  //           <b>{t("Item Description")}:</b> {getItemDescriptions()}
  //         </div>
  //         <div className="detail-stat">
  //           <b>{t("N.T")}:</b> {Number(data.totalNetWt || 0).toFixed(2)}g
  //         </div>
  //         <div className="detail-stat">
  //           <b>{t("G.W.T")}:</b> {Number(data.totalGrossWt || 0).toFixed(2)}g
  //         </div>
  //         <div className="detail-count">
  //           <b>{t("Total Items")}:</b> <span className="count-val-inline">{totalItems}</span>
  //         </div>
  //       </div>
  //     </div>
  ...
  //         <b>{t("Rules")}:</b> 
  //         <ol>
  //           <li>{t("Renew receipt every 6 months to pay interest.")}</li>
  //           <li>{t("Original receipt is required during redemption.")}</li>
  //           <li>{t("for")} {shopSettings?.shop_name || "Pawn Shop"}</li>
  //         </ol>
  //       </div>
  //       <div className="signatures-area">
  //         <div className="sig-line">
  //           <div className="line"></div>
  //           <span>{t("Pledge Signature")}</span>
  //         </div>
  //         <div className="sig-line">
  //           <div className="line"></div>
...
/* ════════════════════════════════════════════════════════
     MAIN RECEIPT — UPDATED LAYOUT WITH TRANSLATIONS & RECEIPT NO
     ════════════════════════════════════════════════════════ */
  const renderMainReceipt = (copyLabelKey) => (
    <div className="sketch-receipt">
  ...
  const renderPocketCopy = () => (
    <div className="pocket-section">
      <div className="pocket-cut-line">✂ ─────────────────────── {t("CUT HERE")} ───────────────────────── ✂</div>
      <div className="pocket-content">
        <div className="pocket-shop-info">
          <div className="pocket-shop-name">{shopSettings?.shop_name || "PAWN SHOP"}</div>
          <div className="pocket-phone">{shopSettings?.phone || ""}</div>
          <div className="pocket-details-list">
            <div className="pocket-detail-row"><span>{t("Receipt No")}:</span><span>{receiptNumber}</span></div>
            <div className="pocket-detail-row"><span>{t("Pledge No")}:</span><span>{data.pledgeNo}</span></div>
            <div className="pocket-detail-row"><span>{t("Pocket No")}:</span><span className="pocket-no">{getPocketNumber()}</span></div>
            <div className="pocket-detail-row"><span>{t("Name")}:</span><span>{data.customer?.name || "—"}</span></div>
            <div className="pocket-detail-row"><span>{t("Date")}:</span><span>{getPledgeDate()}</span></div>
            <div className="pocket-detail-row highlight-row"><span>{t("Loan Amount")}:</span><span>₹ {Number(data.finalLoan || 0).toLocaleString()}</span></div>
          </div>
        </div>
        <div className="pocket-items-section">
          <div className="pocket-items-title">{t("Items List")} <span className="item-count">({totalItems})</span></div>
          <ul className="pocket-items-list">
            {data.items?.map((item, idx) => (
              <li key={idx}>
                {item._typeName} {item.description ? `— ${item.description}` : ""} — {Number(item.net || 0).toFixed(2)}g
        ...
        <div className="pocket-vertical-strip">
          <div className="vertical-item">{t("No")}: {getPocketNumber()}</div>
          <div className="vertical-item">{t("Date")}: {getPledgeDate()}</div>
          <div className="vertical-item">₹ {Number(data.finalLoan || 0).toLocaleString()}/-</div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="print-modal-overlay">
      <div className="print-modal-container">
        <div className="print-action-bar no-print">
          <span>{isReprint ? `🔁 ${t("Duplicate Receipt")}` : `🧾 ${t("Pledge Receipt")}`} — {data.pledgeNo}</span>
          <div className="action-buttons">
