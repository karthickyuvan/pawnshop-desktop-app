...
// src/components/investors/InvestorInvestmentModal.jsx
...
export default function InvestorInvestmentModal({
  ...
  const handleSubmit = () => {
    if (!formData.investor_id) {
      toast.error(t("select_investor_alert", "Please Select Investor")); 
      return;
    }
    if (!formData.amount || Number(formData.amount) <= 0) {
      toast.error(t("invalid_input", "Enter Valid Amount")); 
      return;
    }
    ...
    if (formData.payment_method !== "CASH" && !formData.transaction_ref.trim()) {
      toast.error(t("reference_required", "Please enter a transaction reference")); 
      return;
    }
    ...
    onSave(payload); // ── பேரண்ட் காம்போனென்ட் இதற்கான சக்சஸ் மெசேஜை வெளியிடும் ──
