...
//   const totalPledgedNet = metals.reduce((sum, m) => sum + (m.pledged_net_weight || 0), 0);
//   const totalPledgedGross = metals.reduce((sum, m) => sum + (m.pledged_gross_weight || 0), 0);
//   const totalReleasedNet = metals.reduce((sum, m) => sum + (m.closed_net_weight || 0), 0);
//   const totalReleasedGross = metals.reduce((sum, m) => sum + (m.closed_gross_weight || 0), 0);
...
//   const isProfit = activeYearData.net_profit >= 0;
...
//             <StatCard icon="🗂" label={`${t("total_pledges")} (${selectedYear})`} value={activeYearData.total_pledges.toLocaleString()} color="card-blue" />
//             <StatCard icon="🔨" label={t("auctioned_pockets_lbl", "Pockets Auctioned")} value={activeYearAuction.total_auctioned_pockets.toLocaleString()} color="card-orange" />
...
//             <StatCard icon="📅" label={t("todays_interest", "Interest Collected")} value={formatCurrency(activeYearData.interest_income)} color="card-teal" />
//             <StatCard icon="🏷️" label={t("processing_fee", "Processing Fees")} value={formatCurrency(activeYearData.processing_fees)} color="card-purple" />
//             <StatCard icon="⚖️" label={t("auction_margin_lbl", "Auction Spread Margin")} value={formatCurrency(activeYearData.auction_surplus_deficit)} color={activeYearData.auction_surplus_deficit >= 0 ? "card-teal" : "card-orange"} />
//             <StatCard icon="💹" label={t("misc_income_lbl", "Other Revenue Income")} value={formatCurrency(activeYearData.other_income)} color="card-yellow" />
//             <StatCard icon="🧾" label={t("total_expense", "Operating Expenses")} value={formatCurrency(activeYearData.expenses)} color="card-orange" />
//             <StatCard icon="📈" label={t("total_annual_revenue_lbl", "Total Annual Revenue")} value={formatCurrency(totalRevenue)} color="card-green" />
//             <StatCard icon="💰" label={t("net_retained_profit_lbl", "Net Retained Profit")} value={formatCurrency(activeYearData.net_profit)} color={isProfit ? "card-teal" : "card-orange"} />
//             <StatCard icon="📥" label={t("vault_metal_in_gross_lbl", "Vault Metal In Gross")} value={formatWeight(totalPledgedGross)} color="card-green" />
//             <StatCard icon="📥" label={t("vault_metal_in_net_lbl", "Vault Metal In Net")} value={formatWeight(totalPledgedNet)} color="card-teal" />
//             <StatCard icon="📤" label={t("vault_metal_out_gross_lbl", "Vault Metal Out Gross")} value={formatWeight(totalReleasedGross)} color="card-orange" />
//             <StatCard icon="📤" label={t("vault_metal_out_net_lbl", "Vault Metal Out Net")} value={formatWeight(totalReleasedNet)} color="card-rose" />
//           </div>
...
//                     <td className="text-right text-danger font-bold">({formatCurrency(activeYearData.expenses)})</td>
//                   </tr>
...
//                         {formatCurrency(activeYearData.net_profit)}
//                       </strong>
//                     </td>
//                   </tr>
//                 </tbody>
...
//           {/* ── MIDDLE SECTION: Annual Bank Refinancing Operations (Audit-Ready Layout) ── */}
...
//               {t("refinancing_summary_desc", "Monitors capital flows received from bank mappings versus funding payments returned to the banks.")}
//             </p>
//             <table className="pnl-statement-table">
//               <thead>
//                 <tr>
...
//           {/* ── LOWER COMPONENT: Vault Metal Weight Balance Matrix Breakdown ── */}
    ...
    <div className="yearly-page animate-fade-in">
      ...
      {!loading && (
        <>
          ...
          {/* ── NEW SECTION: Annual Investor Capital Activity & Financing ── */}
          ...
          {/* ── Vault Metal Weight Balance Matrix Breakdown ── */}
          <div className="metal-movement-section" style={{ marginTop: "30px" }}>
            <div className="section-title" style={{ paddingBottom: "10px", borderBottom: "2px solid #cbd5e1", marginBottom: "15px" }}>
              {t("vault_commodity_balance_sheet_title", "Vault Commodity Weight Inventory Balance Sheet")}
            </div>
